export const allGalleryImages = [
    { full: '/assets/images/uraz-teknik.jpg', thumb: '/assets/images/uraz-teknik.jpg', alt: 'URAZ TEKNİK' },
    { full: '/assets/images/uraz-arac-foto.jpeeg', thumb: '/assets/images/uraz-arac-foto.jpeeg', alt: 'Uraz Teknik Hizmet Aracı' },
    { full: '/assets/images/uraz-klima-1.jpeg', thumb: '/assets/images/uraz-klima-1.jpeg', alt: 'Uraz Teknik Klima' },
    { full: '/assets/images/uraz-klima.jpeg', thumb: '/assets/images/uraz-klima.jpeg', alt: 'Uraz Teknik Klima' },
    { full: '/assets/images/uraz-kombi.jpeg', thumb: '/assets/images/uraz-kombi.jpeg', alt: 'Uraz Teknik Kombi' },

    { full: '/assets/images/atakum-drenaj-pompasi.jpg', thumb: '/assets/images/atakum-drenaj-pompasi.jpg', alt: 'Atakum Drenaj Pompası' },

    { full: '/assets/images/canik-klima-bakim.jpg', thumb: '/assets/images/canik-klima-bakim.jpg', alt: 'Canik Klima Bakım' },
    { full: '/assets/images/canik-klima-bakim2.jpg', thumb: '/assets/images/canik-klima-bakim2.jpg', alt: 'Canik Klima Bakım 2' },
    { full: '/assets/images/canik-klima-bakim5.jpg', thumb: '/assets/images/canik-klima-bakim5.jpg', alt: 'Canik Klima Bakım 5' },


    { full: '/assets/images/ilkadim-klima-bakim.jpg', thumb: '/assets/images/ilkadim-klima-bakim.jpg', alt: 'İlkadım Klima Bakım' },
    { full: '/assets/images/ilkadim-klima-bakim6.jpg', thumb: '/assets/images/ilkadim-kombi-bakim.jpg', alt: 'İlkadım Klima Bakım 6' },


    { full: '/assets/images/tekkekoy-klima-bakim.jpg', thumb: '/assets/images/tekkekoy-klima-bakim.jpg', alt: 'Tekkeköy Klima Bakım' },
    { full: '/assets/images/tekkekoy-klima-bakim2.jpg', thumb: '/assets/images/tekkekoy-klima-bakim2.jpg', alt: 'Tekkeköy Klima Bakım 2' },

    { full: '/assets/images/atakum-klima-bakim-temizleme-2.jpg', thumb: '/assets/images/atakum-klima-bakim-temizleme-2.jpg', alt: 'Atakum Klima Bakım Temizleme 2' },
    { full: '/assets/images/atakum-klima-bakim-temizleme.jpg', thumb: '/assets/images/atakum-klima-bakim-temizleme.jpg', alt: 'Atakum Klima Bakım Temizleme' },
    { full: '/assets/images/atakum-klima-bakim-temizleme2.jpg', thumb: '/assets/images/atakum-klima-bakim-temizleme2.jpg', alt: 'Atakum Klima Bakım Temizleme 2' },
    { full: '/assets/images/atakum-klima-bakim2.jpg', thumb: '/assets/images/atakum-klima-bakim2.jpg', alt: 'Atakum Klima Bakım Temizleme 3' },
    { full: '/assets/images/atakum-klima-bakim3.jpg', thumb: '/assets/images/atakum-klima-bakim3.jpg', alt: 'Atakum Klima Bakım Temizleme 4' },
    { full: '/assets/images/atakum-klima-bakim4.jpg', thumb: '/assets/images/atakum-klima-bakim4.jpg', alt: 'Atakum Klima Bakım Temizleme 5' },
    { full: '/assets/images/atakum-klima-bakim5.jpg', thumb: '/assets/images/atakum-klima-bakim5.jpg', alt: 'Atakum Klima Bakım Temizleme 5' },
    { full: '/assets/images/atakum-klima-bakim6.jpg', thumb: '/assets/images/atakum-klima-bakim6.jpg', alt: 'Atakum Klima Bakım Temizleme 7' },
    { full: '/assets/images/atakum-klima-bakim7.jpg', thumb: '/assets/images/atakum-klima-bakim7.jpg', alt: 'Atakum Klima Bakım Temizleme 7' },
    { full: '/assets/images/atakum-klima-bakim.jpg', thumb: '/assets/images/atakum-klima-bakim.jpg', alt: 'Atakum Klima Bakım' },

    { full: '/assets/images/atakum-kombi-bakim-8.jpeg', thumb: '/assets/images/atakum-kombi-bakim-8.jpeg', alt: 'Atakum Kombi Bakım' },
    { full: '/assets/images/atakum-kombi-bakim-9.jpeg', thumb: '/assets/images/atakum-kombi-bakim-9.jpeg', alt: 'Atakum Kombi Bakım' },
    { full: '/assets/images/atakum-kombi-bakim-10.jpeg', thumb: '/assets/images/atakum-kombi-bakim-10.jpeg', alt: 'Atakum Kombi Bakım' },
    { full: '/assets/images/atakum-kombi-bakim-11.jpeg', thumb: '/assets/images/atakum-kombi-bakim-11.jpeg', alt: 'Atakum Kombi Bakım' },
    { full: '/assets/images/atakum-kombi-bakim-12.jpeg', thumb: '/assets/images/atakum-kombi-bakim-12.jpeg', alt: 'Atakum Kombi Bakım' },
    { full: '/assets/images/atakum-kombi-bakim-13.jpeg', thumb: '/assets/images/atakum-kombi-bakim-13.jpeg', alt: 'Atakum Kombi Bakım' },

    { full: '/assets/images/canik-klima-bakim3.jpg', thumb: '/assets/images/canik-klima-bakim3.jpg', alt: 'Canik Klima Bakım 3' },
    { full: '/assets/images/canik-klima-bakim4.jpg', thumb: '/assets/images/canik-klima-bakim4.jpg', alt: 'Canik Klima Bakım 4' },
    { full: '/assets/images/canik-klima-bakim6.jpg', thumb: '/assets/images/canik-klima-bakim6.jpg', alt: 'Canik Klima Bakım 6' },
    { full: '/assets/images/canik-klima-bakim7.jpg', thumb: '/assets/images/canik-klima-bakim7.jpg', alt: 'Canik Klima Bakım 7' },
    { full: '/assets/images/canik-klima-bakim8.jpg', thumb: '/assets/images/canik-klima-bakim8.jpg', alt: 'Canik Klima Bakım 8' },
    { full: '/assets/images/canik-klima-bakim9.jpg', thumb: '/assets/images/canik-klima-bakim9.jpg', alt: 'Canik Klima Bakım 9' },
    { full: '/assets/images/canik-klima-bakim10.jpg', thumb: '/assets/images/canik-klima-bakim10.jpg', alt: 'Canik Klima Bakım 9' },
    { full: '/assets/images/canik-klima-bakim11.jpg', thumb: '/assets/images/canik-klima-bakim11.jpg', alt: 'Canik Klima Bakım 9' },

    { full: '/assets/images/ilkadim-klima-bakim2.jpg', thumb: '/assets/images/ilkadim-klima-bakim2.jpg', alt: 'İlkadım Klima Bakım 2' },
    { full: '/assets/images/ilkadim-klima-bakim3.jpg', thumb: '/assets/images/ilkadim-klima-bakim3.jpg', alt: 'İlkadım Klima Bakım 3' },
    { full: '/assets/images/ilkadim-klima-bakim4.jpg', thumb: '/assets/images/ilkadim-klima-bakim4.jpg', alt: 'İlkadım Klima Bakım 4' },
    { full: '/assets/images/ilkadim-klima-bakim5.jpg', thumb: '/assets/images/ilkadim-klima-bakim5.jpg', alt: 'İlkadım Klima Bakım 5' },
    { full: '/assets/images/ilkadim-klima-bakim7.jpg', thumb: '/assets/images/ilkadim-klima-bakim7.jpg', alt: 'İlkadım Klima Bakım 7' },
    { full: '/assets/images/ilkadim-klima-bakim8.jpg', thumb: '/assets/images/ilkadim-klima-bakim8.jpg', alt: 'İlkadım Klima Bakım 7' },
    { full: '/assets/images/ilkadim-klima-bakim9.jpg', thumb: '/assets/images/ilkadim-klima-bakim9.jpg', alt: 'İlkadım Klima Bakım 7' },
    { full: '/assets/images/ilkadım-kombi-bakim-1.jpeg', thumb: '/assets/images/ilkadım-kombi-bakim-1.jpeg', alt: 'İlkadım Kombi Bakım ' },
    { full: '/assets/images/ilkadım-kombi-bakim-2.jpeg', thumb: '/assets/images/ilkadım-kombi-bakim-2.jpeg', alt: 'İlkadım Kombi Bakım ' },
    { full: '/assets/images/ilkadım-kombi-bakim-3.jpeg', thumb: '/assets/images/ilkadım-kombi-bakim-3.jpeg', alt: 'İlkadım Kombi Bakım ' },

    { full: '/assets/images/tekkekoy-klima-bakim3.jpg', thumb: '/assets/images/tekkekoy-klima-bakim3.jpg', alt: 'Tekkeköy Klima Bakım 3' },
    { full: '/assets/images/tekkekoy-klima-bakim4.jpg', thumb: '/assets/images/tekkekoy-klima-bakim4.jpg', alt: 'Tekkeköy Klima Bakım 4' },
    { full: '/assets/images/tekkekoy-klima-bakim5.jpg', thumb: '/assets/images/tekkekoy-klima-bakim5.jpg', alt: 'Tekkeköy Klima Bakım 5' },
    { full: '/assets/images/tekkekoy-klima-bakim6.jpg', thumb: '/assets/images/tekkekoy-klima-bakim6.jpg', alt: 'Tekkeköy Klima Bakım 6' },
    { full: '/assets/images/tekkekoy-klima-bakim7.jpg', thumb: '/assets/images/tekkekoy-klima-bakim7.jpg', alt: 'Tekkeköy Klima Bakım 7' },
    { full: '/assets/images/tekkekoy-klima-bakim8.jpg', thumb: '/assets/images/tekkekoy-klima-bakim8.jpg', alt: 'Tekkeköy Klima Bakım 8' },
    { full: '/assets/images/tekkekoy-klima-bakim9.jpg', thumb: '/assets/images/tekkekoy-klima-bakim9.jpg', alt: 'Tekkeköy Klima Bakım 9' },
    { full: '/assets/images/tekkekoy-klima-bakim10.jpg', thumb: '/assets/images/tekkekoy-klima-bakim10.jpg', alt: 'Tekkeköy Klima Bakım 10' },
    { full: '/assets/images/tekkekoy-klima-bakim11.jpg', thumb: '/assets/images/tekkekoy-klima-bakim11.jpg', alt: 'Tekkeköy Klima Bakım 10' },


];

/**
 * Bir diziyi yerinde karıştırmak için Fisher-Yates (aka Knuth) Shuffle algoritması.
 * @param {Array} array Karıştırılacak dizi.
 */
function shuffleArray(array) {
    const newArray = [...array];
    for (let i = newArray.length - 1; i > 0; i--) {
        const j = Math.floor(Math.random() * (i + 1));
        [newArray[i], newArray[j]] = [newArray[j], newArray[i]];
    }
    return newArray;
}

// Ana sayfada gösterilecek görselleri dinamik olarak oluşturalım
function createMainPageImages() {
    // İlk (sabit) resmi diziden ayır
    const firstImage = allGalleryImages[0];

    // Geri kalan resimleri al
    const restOfTheImages = allGalleryImages.slice(1);

    // Geri kalan resimleri karıştır
    const shuffledRest = shuffleArray(restOfTheImages);

    // Ana sayfada toplam 6 resim göstereceğimizi varsayalım.
    // Karıştırılmış resimlerden ilk 5 tanesini al.
    const randomImages = shuffledRest.slice(0, 8);

    // Sabit ilk resmi, rastgele seçilen 5 resmin başına ekle
    return [firstImage, ...randomImages];
}

// Ana sayfa için görseller (her sayfa yenilendiğinde farklı olacak)
export const mainPageImages = createMainPageImages();

// Galeri sayfasında tüm görselleri göstermek için (sıralı)
export const allImages = allGalleryImages;